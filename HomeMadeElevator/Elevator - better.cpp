...
{
	while (
		((commandsRegister.top().getDirection() == 1 && upRegister[commandsRegister.top().getFloor()] == 0 ||
		commandsRegister.top().getDirection() == 0 && goRegister[commandsRegister.top().getFloor()] == 0 || 
		commandsRegister.top().getDirection() == -1 && downRegister[commandsRegister.top().getFloor()] == 0) ||
...
void Elevator::plusOneSecond(int& time)
{
	...
	{					// the elevator doesn really care about anything if it is between two floors though
		...
		if (currentFloor != destinationFloor) // if the destination floor hasnt been reached yet
		{
			if (currentDirection == 1) // if the elevator is going up
			{
				if (upRegister[currentFloor] == 1)
				{
					// it takes any passengers on the currenFloor if anyone on it wants to go up
					stopOnFloor(time);
				}
			}
			else // current direction is down
			{
				if (downRegister[currentFloor] == 1)
				{
					// it takes any passengers on the currenFloor if anyone on it wants to go down
					stopOnFloor(time);
				}
			}
		}
...
void Elevator::addCommand(Command command) // I believe having a copy instead of the actual thing would be better
{
	...
	{
		upRegister[flo] = 1; // record that the elevator should pick someone from destinfloor going up
	}
	else
	{
		if (command.getDirection() == -1) // record that the elevator should pick someone from destinfloor going down
		{
			downRegister[flo] = 1; // record that the elevator should pick someone from destinfloor going down
		}
		else
		{
...
{
	downRegister = new bool [floorNum + 1]();
	goRegister = new bool [floorNum + 1]();
	upRegister = new bool [floorNum + 1]();
}
//
...
void Elevator::stopOnFloor(int time)
{
	...
	if (downRegister[currentFloor])
	{
		downRegister[currentFloor] = 0;
		flag = 1;
	}
	if (upRegister[currentFloor])
	{
		upRegister[currentFloor] = 0;
		flag = 1;
	}
